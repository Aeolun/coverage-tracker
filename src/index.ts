...
createConnection().then(async connection => {
  ...
  app.get<ProjectParams>('/coverage/:projectName/:testName/check', async (req, res) => {
    if (hasRequiredParams(req.query)) {
      const newCoveragePercent = Math.round((parseInt(req.query.coveredStatements) + parseInt(req.query.coveredConditionals) + parseInt(req.query.coveredMethods)) / (parseInt(req.query.statements) + parseInt(req.query.conditionals) + parseInt(req.query.methods))*1000)/1000
